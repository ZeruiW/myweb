...
function ProjectCard({ project, featured = false }: { project: (typeof projectsData)[0]; featured?: boolean }) {
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border transition-all duration-300 hover:shadow-xl ${
        featured
          ? 'border-primary-200 bg-gradient-to-br from-white to-primary-50/30 dark:border-primary-800/50 dark:from-gray-900 dark:to-primary-950/20'
    ...
    >
        ...
        <div className="relative aspect-video overflow-hidden bg-gray-100 dark:bg-gray-800">
          ...
          {project.github && (
            <a
              ...
              className="absolute bottom-4 right-4 flex items-center gap-2 rounded-lg bg-white/90 px-3 py-1.5 text-sm font-medium text-gray-900 opacity-0 backdrop-blur-sm transition-all duration-300 hover:bg-white group-hover:opacity-100"
            >
      ...
      <div className="p-6">
        ...
        {/* Title */}
        <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 group-hover:text-primary-600 dark:group-hover:text-primary-400">
          {project.href ? (
            <a href={project.href} target="_blank" rel="noopener noreferrer" className="hover:underline">
              {project.title}
            </a>
          ) : (
            project.title
        ...
        {project.tags && (
          <div className="mt-4 flex flex-wrap gap-2">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600 transition-colors hover:bg-primary-100 hover:text-primary-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-primary-900/50 dark:hover:text-primary-300"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="mt-5 flex items-center justify-between border-t border-gray-100 pt-4 dark:border-gray-800">
          ...
          {project.citations !== undefined && project.citations > 0 && (
            <span className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-500">
              ...
              <strong className="text-primary-600 dark:text-primary-400">{project.citations}</strong> citations
            </span>
          )}
          {project.citations === 0 && (
...
export default function Projects() {
  ...
  const featuredProjects = projectsData.filter(p =>
    p.venue?.includes('A*') || p.venue?.includes('Q1')
  )
      ...
      <div className="space-y-4 pt-6 pb-8 md:space-y-5">
        ...
        <div className="mt-6 flex flex-wrap gap-6">
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold text-primary-600 dark:text-primary-400">{projectsData.length}</span>
            <span className="text-sm text-gray-600 dark:text-gray-400">Projects</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold text-primary-600 dark:text-primary-400">{totalCitations}</span>
            <span className="text-sm text-gray-600 dark:text-gray-400">Total Citations</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold text-primary-600 dark:text-primary-400">{featuredProjects.length}</span>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <span className="rounded-full bg-primary-100 px-4 py-1.5 text-sm font-medium text-primary-700 dark:bg-primary-900/50 dark:text-primary-300">
            Explainable AI
          </span>
